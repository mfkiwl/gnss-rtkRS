...
impl LSQState {
    ...
    pub fn update(&mut self, input: &Input, w: &DMatrix<f64>) -> Result<(), Error> {
...
impl KFState {
    ...
    pub fn update(&mut self, input: &Input, w: &DMatrix<f64>) -> Result<(), Error> {
        ...
        if self.nth == 0 {
            ...
            let g_prime = input.g.transpose();

            let p_bn_inv = p_bn.try_inverse().ok_or(Error::MatrixInversionError)?;

            let w_g = g_prime.clone() * w * input.y.clone();
            let w_gy_pbn = w_g + (p_bn_inv.clone() * x_bn);

            self.p = (g_prime.clone() * w * input.g.clone() + p_bn_inv.clone())
                .try_inverse()
                .ok_or(Error::MatrixInversionError)?;
...
impl FilterState {
    ...
    pub(crate) fn nth(&self) -> u64 {
        match self {
            Self::Kf(state) => state.nth,
            Self::Lsq(state) => state.nth,
        }
    }
    pub fn update(&mut self, input: &Input, w: &DMatrix<f64>) -> Result<(), Error> {
        assert!(input.g.nrows() == w.ncols(), "invalid w matrix");
        assert!(input.g.nrows() == w.nrows(), "invalid w matrix");
        match self {
